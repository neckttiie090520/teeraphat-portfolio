...
/* Adapted from Bencho's MIT-licensed Carousel (bencho.dev).
...
export function Carousel({
  ...
  const activeRef = useRef(OPENS_ON);
  ...
  const paint = useCallback(() => {
    ...
    const next = ((Math.round(turn.current) % N) + N) % N;
  ...
  const down = (e: React.PointerEvent) => {
    ...
    e.currentTarget.removeAttribute('data-suppress-preview');
    cancelAnimationFrame(raf.current);
    drag.current = {
  ...
  const up = (e: React.PointerEvent) => {
    ...
    if (g.moved) {
      ...
      e.currentTarget.setAttribute('data-suppress-preview', 'true');
      glide(to);
    }
  };
  ...
  if (still) {
    return <div className="book-carousel book-carousel--still" aria-label="Actually Faster? book pages">
      {SHOTS.map((shot, index) => <figure key={shot.name}>
        <img data-previewable src={shot.src} alt={shot.alt} loading="lazy" />
        <figcaption><span>{String(index + 1).padStart(2, '0')} / {String(N).padStart(2, '0')}</span>{shot.caption}</figcaption>
      </figure>)}
    </div>;
  }

  return (
    <div className="book-carousel" ref={host} aria-label="Actually Faster? book preview carousel">
    <div className="book-carousel__window" style={{ height: STAGE_H * scale }}>
    <div className="car" style={{ width: STAGE_W, height: STAGE_H, transform: `scale(${scale})` }}>
      <div
        className="car-track"
        data-held={held}
        role="group"
    ...
    <div className="book-carousel__caption">
      <span>{String(active + 1).padStart(2, '0')} / {String(N).padStart(2, '0')}</span>
      <p>{SHOTS[active].caption}</p>
    </div>
    <p className="book-carousel__hint">Swipe to browse · Tap a page to enlarge</p>
    </div>
  );
}
...
function Card({
  ...
  return (
    ...
    >
      <img data-previewable src={shot.src} alt={shot.alt} loading="lazy" draggable={false} />
